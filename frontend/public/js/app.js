...
window.initializeApp = () => {
    try {
        ...
        if(typeof initCNPJLookup === 'function') initCNPJLookup();
        if(typeof initFaturamentoMask === 'function') initFaturamentoMask();
...
function initCalc() {
    ...
    function calculate() {
      try {
        ...
        if (emissaoBase === 0) {
            alert('Para gerar o diagnóstico, você precisa preencher pelo menos um dado de consumo (Energia, Frota, Instalações ou Resíduos).');
            // Retorna ao primeiro passo com campos vazios (Passo 2)
            showStep(2);
            return;
        }
        ...
        // ====== MÉTRICA DQS (DoubleDyn Quality Score) ======
        ...
        let dqsScoreRaw = 1000 - (razaoSetorial * 500);
        const dqsScore = Math.min(1000, Math.max(0, Math.round(dqsScoreRaw)));
        ...
        if (dqsScore >= 700) {
            pcrSeal = 'Ouro';
            pcrColor = '#f1c40f';
        } else if (dqsScore >= 400) {
            pcrSeal = 'Prata';
            pcrColor = '#bdc3c7';
        }
        
        window._lastDqsScore = dqsScore;
        window._lastPcrSeal = pcrSeal;
        
        const dqsValueEl = document.getElementById('dqsScoreValue');
        if (dqsValueEl) {
            dqsValueEl.textContent = dqsScore;
            dqsValueEl.style.color = pcrColor;
            document.getElementById('pcrSealBadge').textContent = 'Selo ' + pcrSeal;
            document.getElementById('pcrSealBadge').style.backgroundColor = pcrColor;
            document.getElementById('pcrSealBadge').style.color = '#111';
        }
        ...
        // Post para o Backend DoubleDyn via Nginx proxy
        fetch('/api/leads', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                company_name: empresa,
                email: email || 'lead@' + empresa + '.com',
                phone: whatsapp || '',
                emissions: totalEmissoes || 0,
                estimated_cost: totalCompensacao || 0,
                dqs_score: window._lastDqsScore || 0,
                pcr_seal: window._lastPcrSeal || 'Bronze'
            })
...
(function(){
    ...
    function autoPlay() { timer = setInterval(() => goTo(current + 1), 8000); }
